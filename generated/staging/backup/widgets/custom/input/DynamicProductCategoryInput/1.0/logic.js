...
Custom.Widgets.input.DynamicProductCategoryInput = RightNow.Widgets.ProductCategoryInput.extend({ 
     ...
     */
    onChange: function() {
        var value = this._eo.data.hierChain[0],
            requiredFields = [],
            fieldsDisplayed = [];
...
-
		...
		if (value && this.data.js.EmailValidationMapping) {
		RightNow.Event.fire("ValidationChange", true);
		} 
		
		//Call a function in the ask page to set the subject with the selected category text.
		rn_SetSubject(this.instanceID);	
