...
BESTANSWER;
    ...
    public function getSocialObjectCountsByStatusType($socialObjectName, $interval = null, $units = null) {
        ...
        if ($interval && $units) {
            ...
            $units = (int) $units;
            // Find the start date
            $rql = "SELECT date_add(sysdate(), $units, '$interval', 1) AS startDate from SocialUser LIMIT 1";
            try {
                $date = Connect\ROQL::query($rql)->next()->next();
            }
            catch(Connect\ConnectAPIErrorBase $e){
                return $this->getResponseObject(null, null, $e->getMessage());
            }
            $dateFilterRQL = " AND SO.UpdatedTime >= '" . $date['startDate'] . "' " . (($socialObjectName !== 'SocialUser') ? " AND CreatedBySocialUser IS NOT NULL " : "");
        }
        $interfaceFilterRQL = '';
        if ($socialObjectName !== 'SocialUser') {
    ...
    public function getRecentSocialObjectCountsByDateTime($socialObjectName, $interval = 'day', $units = -7) {
        ...
        $rql = "SELECT date_trunc(sysdate(), '$interval') AS endDate, date_add(sysdate(), $units, '$interval', 1) AS startDate from SocialUser LIMIT 1";
