...
class ForumList extends \RightNow\Libraries\Widget\Base {
    ...
    function getData() {
        ...
        if ($this->data['attrs']['sort_order'] !== 'comment_count') {
            $questionCount = $this->CI->model('SocialQuestion')->getQuestionCountByProductCategory($dataType, array_keys($userProducts), $this->data['attrs']['show_columns'], $this->data['attrs']['max_forum_count'], $this->data['attrs']['sort_order'])->result;

            if (in_array('comment_count', $this->data['attrs']['show_columns']) && $questionCount) {
                $commentCount = $this->CI->model('SocialComment')->getCommentCountByProductCategory($dataType, array_keys($questionCount['questionCount']), $this->data['attrs']['max_forum_count'])->result;
            }
        }
        else {
            $commentCount = $this->CI->model('SocialComment')->getCommentCountByProductCategory($dataType, array_keys($userProducts), $this->data['attrs']['max_forum_count'], $this->data['attrs']['sort_order'])->result;
            ...
            $questionCount = $this->CI->model('SocialQuestion')->getQuestionCountByProductCategory($dataType, $prodCats, $this->data['attrs']['show_columns'], $count, $sortOrder)->result;
